...
pub struct LRUCache {
    pub read_syncer: Box<dyn ReadSync>,
...
impl LRUCache {
    ...
    fn commit_merged_node(
        ...
        if self.lru_leaf.capacity == 0 && self.lru_internal.capacity == 0 {
            return Ok(());
        }
...
impl Cache for LRUCache {
    ...
    fn remote_sync<F: ReadSyncFetcher>(
        ...
        let copied_tree = fetcher.fetch_by_cache(
        ...
        let (dst_ptr, subtree) = if copied_tree.is_some() {
            (ptr.clone(), copied_tree.unwrap())
        } else {
