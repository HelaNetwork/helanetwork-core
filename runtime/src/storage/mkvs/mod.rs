...
pub trait FallibleMKVS {
    ...
    /// migrate the k/v from source to target, then discard the source.
    fn migrate(source: &Self, target: &Self) -> Result<()>;
}
...
impl<T: FallibleMKVS + ?Sized> FallibleMKVS for &mut T {
    ...
    where
        F: FnMut (&Vec<u8>, &Vec<u8>) -> (),
    {
        T::multi_insert(self, ctx, kvs, cb)
    }
    ...
    fn multi_commit(&mut self, ctx: Context, namespace: Namespace, version: u64, kvs_dist: &Option<Vec<usize>>) -> Result<Hash> {
        T::multi_commit(self, ctx, namespace, version, kvs_dist)
    }

    fn migrate(source: &Self, target: &Self) -> Result<()> {
        T::migrate(source, target)
    }
}
