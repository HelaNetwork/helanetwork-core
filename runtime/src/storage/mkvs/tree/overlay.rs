...
impl<T: mkvs::FallibleMKVS> OverlayTree<T> {
    ...
    pub fn state_new(inner: T, index: i32) -> Self {
        Self {
            inner,
            overlay: BTreeMap::new(),
            dirty: HashSet::new(),
    ...
    pub fn merge(source: &mut Self, target: &mut Self) {
        let _ = T::migrate(&source.inner, &target.inner);

        for (key, value) in &source.overlay {

            target.overlay.insert(key.to_owned(), value.to_owned());
            target.dirty.insert(key.to_owned());
        }
        for key in &source.dirty {
            target.dirty.insert(key.to_owned());
        }
    }
