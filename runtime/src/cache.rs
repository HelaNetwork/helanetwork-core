...
impl Cache {
    ...
    pub fn build(protocol: &Arc<Protocol>, root: Root) -> Tree {
    ...
    pub fn host_cache_build(protocol: &Arc<Protocol>, root: Root, cache: Arc<Mutex<Option<Tree>>>, index: i32) -> Tree {
        //let config = protocol.get_config();
        let read_syncer = HostReadSyncer::cache_new(protocol.clone(), HostStorageEndpoint::Runtime, cache, index);
        Tree::builder()
            .with_capacity(
                if index > 0 {
                    0
                } else {
                    0   //config.storage.cache_node_capacity
                },
                if index > 0 {
                    0
                } else {
                    0   //config.storage.cache_value_capacity
                },
            )
            .with_root(root)
            .build(Box::new(read_syncer))
