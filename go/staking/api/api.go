...
type ConsensusParameters struct { // nolint: maligned
	...
	// Lambda is a parameter in our coin minting function (a parameter of an exponential distribution)
	LambdaMintingCurve float64 `json:"lambda_minting_curve"`

	// TotalMintedCoins is the total amount of coins to be minted.
	TotalMintedCoins float64 `json:"total_minted_coins"`

	// TODO: add weights for different tasks
	// E.g., RewardWeightValidator
}
...
func (c *ConsensusParameterChanges) Apply(params *ConsensusParameters) error {
	...
	if c.LambdaMintingCurve != nil {
		params.LambdaMintingCurve = *c.LambdaMintingCurve
	}
	if c.TotalMintedCoins != nil {
		params.TotalMintedCoins = *c.TotalMintedCoins
	}
	return nil
}
