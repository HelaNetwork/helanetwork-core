...
func TestMintingAndDistribution(t *testing.T) {
	require := require.New(t)
	...
	err := s.SetTotalSupply(ctx, quantity.NewFromUint64(10E8))
	require.NoError(err, "Failed to set the total supply")
	// Set the lambda of our minting curve
	err = s.SetConsensusParameters(ctx, &staking.ConsensusParameters{
		LambdaMintingCurve: 0.0000256,
	})
	require.NoError(err, "Failed to set the lambda")

	// Simulate a proposer
	proposer, err := memorySigner.NewSigner(rand.Reader)
	require.NoError(err, "generating a proposer")
	proposerAddress := staking.NewAddress(proposer.Public())

	// Simulate validators
	nValidators := 3
	var validatorAddresses []staking.Address
	for i := 0; i < nValidators; i++ {
		validator, err := memorySigner.NewSigner(rand.Reader)
		require.NoError(err, "generating validator", i)
		validatorAddress := staking.NewAddress(validator.Public())
		validatorAddresses = append(validatorAddresses, validatorAddress)
	}

	// Simulate runtime nodes
	var runtimeCommitteeAddresses []staking.Address
	// Simulate Epochs 0 and 1
	nEpochs := uint64(3)
	for e := uint64(0); e < nEpochs; e++ {
		fmt.Println("Epoch", e)
		feesConsensusLayer := quantity.NewFromUint64(e * 100)
		require.NoError(s.AddRewards(
			ctx, 
			beacon.EpochTime(e), 
			feesConsensusLayer, 
			proposerAddress, 
			validatorAddresses,
			runtimeCommitteeAddresses,
			), 
			"add rewards epoch", e)
	}
}