...
// BlockFees returns the accumulated fee balance in the current block.
func EpochFees(ctx *abciAPI.Context) quantity.Quantity {
	// Fetch accumulated fees in the current epoch.
	return ctx.BlockContext().Get(feeAccumulatorKey{}).(*feeAccumulator).balance
}
