...
func (n *Node) SubmitTxNoWait(ctx context.Context, tx []byte) error {
	// Make sure consensus is synced.
	select {
	case <-n.commonNode.Consensus.Synced():
	case <-ctx.Done():
		return ctx.Err()
	default:
		return api.ErrNotSynced
	}

	// Submit transaction to the pool and wait for it to get checked.
	err := n.commonNode.TxPool.SubmitTxNoWait(ctx, tx, &txpool.TransactionMeta{Local: true})
	if err != nil {
		return err
	}
