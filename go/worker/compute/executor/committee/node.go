...
func (n *Node) schedulerCreateProposalLocked(ctx context.Context, inputRoot hash.Hash, txHashes []hash.Hash, txSplits []uint32) (*commitment.Proposal, error) {
...
func (n *Node) startRuntimeBatchSchedulingLocked(
	...
	n.logger.Warn("asking runtime to schedule batch",
...
func (n *Node) handleProcessedBatch(batch *processedBatch, processingCh chan *processedBatch) {
	...
	case state.mode == protocol.ExecutionModeSchedule:
		...
		_, err = n.schedulerCreateProposalLocked(roundCtx, batch.txInputRoot, batch.txHashes, batch.txSplits)
		if err != nil {
