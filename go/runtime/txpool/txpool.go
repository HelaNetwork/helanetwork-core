...
const (
	...
	// maxRepublishTxs is the maximum amount of transactions to republish. Original: 128
	maxRepublishTxs = 10000
	...
	txCheckWaitingThreshold = 1000
	txCheckWaitingTimeout   = 0
)
...
func (t *txPool) checkWorker() {
	...
	if t.nodeMode == "client" || t.nodeMode == "client-stateless" {
		t.logger.Info("txpool running client mode")
	...
	t.logger.Info("txpool running compute mode")

	var timeCh <-chan time.Time
	for {
		sz := t.checkTxQueue.size()
		//state := t.checkWorkerState

		switch t.checkWorkerState {
		case "idle":
			if sz == 0 {
			} else if sz < txCheckWaitingThreshold {
				timeCh = time.After(txCheckWaitingTimeout * time.Millisecond)
				t.checkWorkerState = "pending"
			} else {
				t.checkWorkerState = "checking"
			}
		case "pending":
			if sz == 0 {
				t.checkWorkerState = "idle"
			} else if sz >= txCheckWaitingThreshold {
				t.checkWorkerState = "checking"
			}
		case "checking":
        ...
        //if state != t.checkWorkerState {
        //    t.logger.Info("txpool state change", "from", state, "to", t.checkWorkerState)
        //}
